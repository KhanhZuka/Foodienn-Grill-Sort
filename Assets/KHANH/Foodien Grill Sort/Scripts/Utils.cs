...
namespace KHANH.FoodienGrillSort
{
    public class Utils
    {
        public static List<T> GetListInChild<T>(Transform parent) // ham nay lay ra cac thanh phan cua mot doi tuong cha
        {
        ...
        {
            List<T> result = new List<T>();
            n = Mathf.Min(n, source.Count); // check de dam bao so luong lay ve khong vuot qua so luong list co san
